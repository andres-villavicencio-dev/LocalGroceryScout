...
export const ResultsView: React.FC<ResultsViewProps> = ({ 
  ...
  const formattedText = result.text.split('\n').map((line, i) => {
    ...
    if (isHeader) {
      return <h3 key={i} className="text-lg font-bold text-emerald-800 mt-4 mb-2">{line.replace(/#/g, '').replace(/\*\*/g, '')}</h3>;
    }
    ...
    if (line.trim().startsWith('*') || line.trim().startsWith('-') || /^\d+\./.test(line.trim())) {
        ...
        return (
          <li key={i} className="ml-4 mb-1 list-disc list-inside text-gray-700">
            {parts.map((part, j) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h2 className="text-2xl font-bold text-gray-900">Price Comparison for "{result.productName}"</h2>
            {lowestPrice && (
                <div className="text-emerald-600 font-medium text-sm mt-1">
                    Best Deal: ${lowestPrice.price} at {lowestPrice.store}
                </div>
            )}
        </div>
        ...
        <div className="flex items-center gap-3">
            {lists.length > 0 && (
                <div className="flex items-center bg-white border border-gray-200 rounded-lg p-1 shadow-sm">
                    <select 
                        ...
                        className="text-sm border-none focus:ring-0 text-gray-600 py-1 pl-2 pr-6"
                    >
            ...
            <button
            onClick={onReset}
            className="text-sm text-gray-500 hover:text-gray-800 font-medium flex items-center px-3 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
            >
      ...
      {history && result.productName && (
        <PriceHistoryChart history={history} productName={result.productName} />
      )}
