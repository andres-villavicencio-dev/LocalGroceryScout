      ...
      <div className="flex overflow-x-auto gap-2 mb-6 pb-2 scrollbar-hide">
        {lists.map(list => (
          <button
            ...
            className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-medium transition-colors ${activeList?.id === list.id
          ...
          <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50 dark:bg-gray-700/50">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center px-4 py-2 rounded-lg font-medium text-sm transition-all ${isScouting
                ...
                {sortedItems.length === 0 ? (
                  ...
                  (() => {
                    const grouped = sortedItems.reduce((acc, item) => {
                      const store = item.bestStore || 'Unassigned';
                      if (!acc[store]) acc[store] = [];
                      acc[store].push(item);
                      return acc;
                    }, {} as Record<string, ShoppingListItem[]>);

                    // Sort stores: Unassigned last, others alphabetical or by total price?
                    // Let's put Unassigned last, others alphabetical for now.
                    const storeNames = Object.keys(grouped).sort((a, b) => {
                    ...
                    return storeNames.map(storeName => {
                      ...
                      const storeTotal = items.reduce((sum, i) => sum + (i.bestPrice || 0), 0);

                      return (
                        <div key={storeName} className="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4 border border-gray-100 dark:border-gray-700">
                          <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-200 dark:border-gray-600">
                            <h4 className="font-bold text-gray-800 dark:text-white flex items-center">
                              {storeName === 'Unassigned' ? '📝 Unassigned' : `🏪 ${storeName}`}
