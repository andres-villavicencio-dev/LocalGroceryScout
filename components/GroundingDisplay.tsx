          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {mapChunks.map((chunk, idx) => (
              <a
                ...
                className="flex items-start p-3 rounded-lg border border-gray-200 hover:border-emerald-400 hover:bg-emerald-50 transition-colors duration-200 bg-white shadow-sm group"
              >
                <div className="flex-1">
                  <p className="text-sm font-semibold text-gray-800 group-hover:text-emerald-700">
                    {chunk.maps?.title || "Unknown Location"}
                  </p>
                  <p className="text-xs text-gray-500 mt-1">View on Maps</p>
                </div>
            ...
            {webChunks.map((chunk, idx) => (
              <li key={`web-${idx}`}>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block p-2 rounded hover:bg-gray-100 text-sm text-blue-600 hover:underline truncate"
                >
